...
double *he_read_healpix_map(char *fname,long *nside,int nfield)
{
  ...
  map=my_malloc(npix*sizeof(double));
  ...
  if(nested_in_file) {
    ...
    map_ring=my_malloc(npix*sizeof(double));
...
{
  long npix=12*nside*nside;
  double *map_out=my_malloc(npix*sizeof(double));
...
#pragma omp for
  ...
  memcpy(map_in,map_out,npix*sizeof(double));
...
void he_udgrade(double *map_in,long nside_in,
		double *map_out,long nside_out,
		int nest)
{
...
void he_in_ring(int nside,int iz,double phi0,double dphi,
		int *listir,int *nir)
{
...
void he_query_disc(int nside,double cth0,double phi,double radius,
...
static void sht_wrapper(int spin,int lmax,int nside,int ntrans,double **maps,complex **alms,int alm2map)
{
  double time=0;
...
void he_map2alm(int nside,int lmax,int ntrans,int spin,double **maps,complex **alms,int niter)
{
  ...
  if(niter) {
    ...
    double **maps_2=my_malloc(ntrans*nmaps*sizeof(double *));
    complex **alms_2=my_malloc(ntrans*nmaps*sizeof(complex *));

    for(ii=0;ii<ntrans*nmaps;ii++) {
      maps_2[ii]=my_malloc(npix*sizeof(double));
      alms_2[ii]=my_malloc(nalm*sizeof(complex));
    }
...
void he_alm2cl(complex **alms_1,complex **alms_2,
	       int pol_1,int pol_2,
	       double **cls,int lmax)
{
  int i1,index_cl;
  ...
  for(i1=0;i1<nmaps_1;i1++) {
    int i2;
    complex *alm1=alms_1[i1];
    for(i2=0;i2<nmaps_2;i2++) {
      int l;
      complex *alm2=alms_2[i2];
      for(l=0;l<=lmax;l++) {
	int m;
...
void he_anafast(double **maps_1,double **maps_2,
		int pol_1,int pol_2,double **cls,
		int nside,int lmax,int iter)
{
  complex **alms_1,**alms_2;
  int i1,lmax_here=3*nside-1;
  int nmaps_1=1, nmaps_2=1;
  if(pol_1) nmaps_1=2;
  if(pol_2) nmaps_2=2;

  alms_1=my_malloc(nmaps_1*sizeof(complex *));
  for(i1=0;i1<nmaps_1;i1++)
    alms_1[i1]=my_malloc(he_nalms(lmax_here)*sizeof(complex));
  he_map2alm(nside,lmax,1,2*pol_1,maps_1,alms_1,iter);

  if(maps_1==maps_2)
    alms_2=alms_1;
  else {
    alms_2=my_malloc(nmaps_2*sizeof(complex *));
    for(i1=0;i1<nmaps_2;i1++)
      alms_2[i1]=my_malloc(he_nalms(lmax_here)*sizeof(complex));
...
complex **he_synalm(int nside,int nmaps,int lmax,double **cells,double **beam,int seed)
{
  ...
  alms=my_malloc(nmaps*sizeof(complex *));
  for(imap=0;imap<nmaps;imap++)
    alms[imap]=my_malloc(he_nalms(lmax_here)*sizeof(complex));
...
#pragma omp parallel default(none)				\
  ...
  {
    int ll;
    double *bms=my_malloc(nmaps*sizeof(double));
...
#pragma omp for
	...
	for(imp1=0;imp1<nmaps;imp1++)
	  alms[imp1][he_indexlm(ll,0,lmax_here)]=bms[imp1]*((complex)(M_SQRT2*gsl_vector_get(rv2,imp1)));
	...
	for(mm=1;mm<=ll;mm++) {
	  ...
	  for(imp1=0;imp1<nmaps;imp1++)
	    alms[imp1][index]=bms[imp1]*((complex)(gsl_vector_get(rv2,imp1)+I*gsl_vector_get(iv2,imp1)));
	}
      }
    } //end omp for
    free(bms);
...
#define LMAX_MIN 10.
  ...
  par->b_arr=my_malloc(par->nj*sizeof(double *));
  for(ii=0;ii<par->nj;ii++) {
    par->b_arr[ii]=my_calloc(3*nside0,sizeof(double));
  ...
  int lmx0=(int)(par->b);
  for(ii=0;ii<=lmx0;ii++) {
    double b1=par->b_arr[0][ii];
    double b2=par->b_arr[1][ii];
    double h_here=b1*b1+b2*b2;
    if(h_here<1)
  ...
  for(ii=0;ii<3*nside0;ii++) {
    double b1=par->b_arr[par->nj-2][ii];
    double b2=par->b_arr[par->nj-1][ii];
...
{
  int ii,nmaps;
  double ***nt=my_malloc(par->nj*sizeof(double **));
  if(pol) nmaps=3;
  else nmaps=1;
  ...
  for(ii=0;ii<par->nj;ii++) {
    ...
    nt[ii]=my_malloc(nmaps*sizeof(double *));
    for(imap=0;imap<nmaps;imap++)
      nt[ii][imap]=my_malloc(12*ns*ns*sizeof(double));
  }
			  ...
			  int return_alm,int pol,int input_TEB,int output_TEB)
{
  ...
  //Allocate alms
  alm=my_malloc(nmaps*sizeof(complex *));
  alm_dum=my_malloc(nmaps*sizeof(complex *));
  for(j=0;j<nmaps;j++) {
    alm[j]=my_calloc(n_alms,sizeof(complex));
    alm_dum[j]=my_malloc(n_alms*sizeof(complex));
  }
			  ...
			  int return_alm,int pol,int input_TEB,int output_TEB)
{
  ...
  alm_dum=my_malloc(nmaps*sizeof(complex *));
  for(j=0;j<nmaps;j++) {
    alm[j]=my_malloc(n_alms*sizeof(complex));
    alm_dum[j]=my_malloc(n_alms*sizeof(complex));
  }
