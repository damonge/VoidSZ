...
def read_ydata_single(fname,nx,na) :
    ...
    return xarr,aarr,warr_2d,warr_1d,np.sum((data[3]).reshape([nx,na]),axis=1),np.sum((data[4]).reshape([nx,na]),axis=1)
...
def get_run_stats(prefix,nx,na,nsims) :
    ...
    if not os.path.isfile(fname_stats+".npz") :
        x_d,a_d,w_2d_d,w_1d_d,f_1d_d,h_1d_d=read_ydata_single(dirname+"/wth_voids.txt",nx,na)
        w_2d_ms=[]; w_1d_ms=[]; f_1d_ms=[]; h_1d_ms=[]
        for i in np.arange(nsims) :
            print i
            x,a,w2d,w1d,f1d,h1d=read_ydata_single(dirname+"/wth_mock_%04d.txt"%(i+1),nx,na)
            w_2d_ms.append(w2d)
            w_1d_ms.append(w1d)
            f_1d_ms.append(f1d)
            h_1d_ms.append(h1d)
        ...
        f_1d_ms=np.array(f_1d_ms)
        h_1d_ms=np.array(h_1d_ms)
        w_2d_mean=np.mean(w_2d_ms,axis=0)
        w_2d_std =np.std(w_2d_ms,axis=0)
#        w_1d_mean=np.mean(w_1d_ms,axis=0)
        w_1d_mean=np.sum(f_1d_ms,axis=0)/np.sum(h_1d_ms,axis=0)
